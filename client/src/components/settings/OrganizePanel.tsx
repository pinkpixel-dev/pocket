...
export function OrganizePanel({
  ...
  const [pendingConvertTags, setPendingConvertTags] = useState<
    Collection[] | null
  >(null);
  ...
  const sortedCollections = useMemo(() => {
    ...
    if (sortOrder === "count-desc") {
      return list.sort(
        (a, b) =>
          b.bookmarkCount - a.bookmarkCount || a.name.localeCompare(b.name),
      );
    }
    if (sortOrder === "count-asc") {
      return list.sort(
        (a, b) =>
          a.bookmarkCount - b.bookmarkCount || a.name.localeCompare(b.name),
      );
    }
  ...
  const someCollectionsSelected =
    selectedIds.size > 0 && !allCollectionsSelected;
  ...
  const confirmConvertToTags = async () => {
    ...
    setBusy(true);
    try {
      const res = await api.convertCollectionsToTags(
        pendingConvertTags.map((c) => c.id),
      );
      toast.success(
      ...
      toast.error(
        err instanceof ApiError
          ? err.message
          : "Could not convert collections to tags.",
      );
    } finally {
      setBusy(false);
    ...
    <div className="flex flex-col gap-6">
      <section className="flex flex-col gap-2">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-[0.875rem] text-ink-muted">
              Deleting a collection keeps its bookmarks. They just stop
              belonging to it.
            </p>
          </div>

          {collections.length > 1 ? (
            <div className="flex flex-col gap-2 self-start text-[0.8125rem] sm:items-end sm:self-auto">
              {aiConfigured ? (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => setCleanupOpen(true)}
                  className="self-start text-[0.8125rem] sm:self-end"
        ...
        {collections.length === 0 ? (
          <p className="py-2 text-[0.875rem] text-ink-faint">
            No collections yet.
          </p>
        ) : (
          <>
            <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg px-3 text-[0.8125rem] text-ink-muted">
              <input
                ...
                ref={(el) => {
                  if (el) el.indeterminate = someCollectionsSelected;
                }}
              ...
              {sortedCollections.map((collection) => (
                <li key={collection.id} className={ROW}>
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor:
                        collection.color ?? "var(--color-line-strong)",
                    }}
                  />
